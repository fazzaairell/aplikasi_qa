{{--
    resources/views/components/topbar-developer.blade.php

    Komponen topbar TUNGGAL untuk role Developer.
    Dipakai di layout utama, cukup panggil: <x-topbar-developer />

    Perbaikan dari 3 versi topbar sebelumnya (Dashboard, Bug Tracker, Notifikasi):
    1. Struktur container disamakan: max-w-7xl mx-auto px-8 + 3 kolom w-1/3
       (sebelumnya Dashboard tidak pakai max-w-7xl, jadi logo & nav terlihat
        lebih besar/renggang dibanding halaman lain).
    2. Duplicate atribut `style` pada dropdown notifikasi digabung jadi satu
       (sebelumnya browser mengabaikan style kedua, jadi background dropdown
        bisa gagal ter-render).
    3. Highlight nav "Bug Report" disamakan pakai wildcard routeIs('bugs.*')
       supaya tab tetap aktif saat membuka halaman detail bug.
    4. Query notifikasi (unreadCount, recentNotifs) hanya ada di SATU tempat.
--}}
...
<header
    ...
    <div class="max-w-7xl mx-auto px-8 h-full flex items-center justify-between relative">
        ...
        <div class="hidden md:flex flex-1 justify-center space-x-2 w-1/3">
            ...
            {{-- routeIs('bugs.*') dipakai secara konsisten agar tab tetap
                 aktif di halaman index, history, maupun detail bug --}}
        ...
        <div class="flex items-center justify-end space-x-3 w-1/3">
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                ...
                {{-- Dropdown panel: style digabung jadi satu atribut (fix duplicate style) --}}
                <div x-show="open"
                     ...
                     style="display:none; background:#111827; border-color:rgba(255,255,255,0.07);">
                    ...
                    <a href="{{ route('notifications.timeline') }}"
                       class="block px-4 py-2.5 text-center text-[11px] text-indigo-400 hover:text-indigo-300 font-semibold border-t transition"
                       style="border-color:rgba(255,255,255,0.06);">
