    ...
    <div class="flex flex-col h-full">
        ...
        <div class="p-5 flex items-center justify-between" :class="collapsed ? 'md:justify-center md:px-3' : ''">
            ...
            <div class="flex items-center space-x-3" x-show="!collapsed" x-transition>
                ...
                <span class="font-bold text-base text-white tracking-wide whitespace-nowrap">TESTIFY</span>
            </div>
        ...
        <nav class="flex-1 px-3 space-y-0.5 mt-1 overflow-y-auto">
            @php
                if (auth()->user()->isDeveloper()) {
                    // Developer: hanya Dashboard, Bug Tracker, Notifikasi
                    $navItems = [
                        ['route' => 'dashboard.developer', 'match' => 'dashboard*',      'icon' => 'M4 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zM14 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z', 'label' => 'Dashboard'],
                        ...
                        ['route' => 'notifications.timeline', 'match' => 'notifications.*', 'icon' => 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9', 'label' => 'Notifikasi'],
                    ];
                } else {
                    // Non-developer: menu lengkap
                    $navItems = [
                        ['route' => auth()->user()->isQa() ? 'dashboard.qa' : 'dashboard', 'match' => 'dashboard*',         'icon' => 'M4 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zM14 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z', 'label' => 'Dashboard'],
                        ...
                        ['route' => 'test-suite-templates.index', 'match' => 'test-suite-templates.*', 'icon' => 'M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2', 'label' => 'Template Suite'],
                    ...
                    if (! auth()->user()->isQaTester()) {
                        $navItems[] = ['route' => 'reports.comprehensive', 'match' => 'reports.*', 'icon' => 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'label' => 'Laporan'];
                        $navItems[] = ['route' => 'bugs.index',            'match' => 'bugs.*',    'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', 'label' => 'Bug Tracker'];
                        $navItems[] = ['route' => 'users.index',           'match' => 'users.*',   'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', 'label' => 'Manajemen Pengguna'];
                    } else {
                        $navItems[] = ['route' => 'reports.comprehensive', 'match' => 'reports.*', 'icon' => 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'label' => 'Laporan'];
                        $navItems[] = ['route' => 'bugs.index',            'match' => 'bugs.*',    'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', 'label' => 'Bug Tracker'];
                    }
