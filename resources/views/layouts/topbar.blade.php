...
<body class="h-full font-sans text-slate-100 overflow-y-auto bg-[#0b0f19]" x-data="{}">
    ...
    @if(auth()->check() && auth()->user()->isDeveloper())
        ...
        {{-- Developer: satu sumber kebenaran untuk topbar --}}
        <x-topbar-developer />

    @else

        {{-- Admin / QA Tester: topbar sederhana dengan search --}}
    ...
    <main class="px-6 sm:px-8 pb-8 space-y-6 {{ auth()->check() && auth()->user()->isDeveloper() ? 'max-w-7xl mx-auto w-full' : '' }}">
        @yield('content')
    </main>
