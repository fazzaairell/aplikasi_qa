        ...
        <main class="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div class="mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                @if($unreadCount > 0)
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            class="inline-flex items-center gap-1.5 px-4 py-2 border border-white/[0.1] hover:border-indigo-500/40 text-slate-300 hover:text-white rounded-full text-xs font-semibold transition"
                        >
                            <svg
                                class="w-3.5 h-3.5"
                                fill="none"
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @foreach($readOptions as $value => $label)
                    ...
                    <a
                        href="{{ route('notifications.timeline', array_filter([
                            'read_status' => $value,
                            'type' => request('type')
                        ])) }}"
                ...
                >
                    ...
                    <option
                        value="{{ route('notifications.timeline', array_filter([
            ...
            >
                ...
                @forelse($notifications as $notif)
                    ...
                    @php
                        ...
                        $initials = $causer
                            ? collect(explode(' ', trim($causer->name)))
                        ...
                        $avatarPalette = [
                            ...
                            [
                                'bg' => 'rgba(99,182,120,0.18)',
                                'text' => '#a3d9b1'
                            ],
                            [
                                'bg' => 'rgba(217,151,40,0.18)',
                                'text' => '#f3cd85'
                            ],
                            [
                                'bg' => 'rgba(93,170,202,0.18)',
                                'text' => '#9fd6ef'
                            ],
                        ];

                        $avatarColor = $avatarPalette[
                            ($causer->id ?? $notif->id) % count($avatarPalette)
                        ];

                        $assignee = $notif->bug?->assignee;

                        $assigneeInitials = $assignee
                            ? collect(explode(' ', trim($assignee->name)))
                                ->map(fn($w) => mb_substr($w, 0, 1))
                                ->take(2)
                                ->implode('')
                            : null;
                    ...
                    <div
                        class="flex gap-3.5 px-4 sm:px-5 py-4 {{ !$loop->first ? 'border-t border-white/[0.06]' : '' }}"
                    >

                        {{-- AVATAR --}}
                        <div
                            class="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0"
                            data-bg="{{ $avatarColor['bg'] }}"
                            data-color="{{ $avatarColor['text'] }}"
                            x-init="$el.style.background = $el.dataset.bg; $el.style.color = $el.dataset.color;"
                        >
                            {{ strtoupper($initials) }}
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p
                                class="text-[13.5px] leading-snug {{ !$notif->is_read ? 'font-bold text-white' : 'font-normal text-slate-300' }}"
                            >
                                {{ $notif->message }}
                            ...
                            <p class="text-[11.5px] text-slate-500 mt-1 mb-2.5">
                            ...
                            @if($notif->bug)
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <a
                                        href="{{ route('bugs.show', $notif->bug->id) }}"
                                    ...
                                    @if($assignee)
                                        ...
                                        <span
                                            class="inline-flex items-center gap-1.5 border border-white/[0.1] text-slate-400 text-[11px] pl-1 pr-2.5 py-0.5 rounded-full"
                                        >
                                            <span
                                                class="w-4 h-4 rounded-full bg-white/10 text-slate-200 text-[8px] font-bold flex items-center justify-center flex-shrink-0"
                                            >
                                                {{ strtoupper($assigneeInitials) }}
                                            </span>
                        ...
                        @if(!$notif->is_read)
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    title="Tandai dibaca"
                                    class="text-slate-500 hover:text-indigo-400 transition p-1"
                                >
                                    <svg
                                        class="w-4 h-4"
                                        fill="none"
                ...
                @empty
                    ...
                    <div class="p-12 text-center">
                        <p class="text-slate-400 text-sm">
                            Belum ada notifikasi dengan filter yang dipilih.
                        </p>
                    </div>
